...
## 新增 scripts/tools/record_ik_stack.py

  - 在 tools/record_demos.py 基础上进行修改。
  - 使用分段三次 Hermite 插值（PCHIP）来平滑轨迹。
  - 依托 IK-Rel 环境内置的逆运动学（IK）解算，IKStackController 无需计算关节角度，仅需输出末端位姿增量与夹爪开合指令。环境的 Action Space 维度为 (1, 7)（即 6D 位姿增量 + 1D 夹爪指令）。
